...
function [X_k,S_k,lossflag]=PDA_SRCKF12_clutter(X0,S0,Z_k,utm)
global h q1 q2 q3 sigma sigma_prime gamma lamda lossnum lossnum_PDA
global B
    ...
    if m==1  % no measurement falls in the gate
       ...
       S_k=S_k_k_1;
        ...
        for m=1:1:nc+1                                         %关联概率的计算
             ...
             e(m)=exp(-0.5*vgate(m)'/(S_zz_k_k_1*S_zz_k_k_1')*vgate(m));     
        end
